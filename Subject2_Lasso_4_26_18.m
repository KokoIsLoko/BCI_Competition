...
clear; clc;
load('predicted_dg_v4.mat')

% Get data
session = IEEGSession('I521_Sub2_Training_ecog', 'dweiss', 'Dwe_ieeglogin');
session.openDataSet('I521_Sub2_Training_dg');
session.openDataSet('I521_Sub2_Leaderboard_ecog');

fs = session.data(1).sampleRate;
trainingECoG = session.data(1).getvalues(1:300000,1:48);
nyq = fs/2;
...
for i = 1:48
    ...
    avg125_160Hz = mean(abs(s(25:31,:)));
    avg160_175Hz = mean(abs(s(32:34,:)));
...
testingTrace = session.data(3).getvalues(1:150000,1:48);
...
xq = 50:length(YhatF1)*50;
for i = 1:5
    pred(:,i) = spline([1:length(YhatF1)].*50,Yhat(:,i),xq);
    predicted(:,i) = [zeros(50*N,1);pred(:,i);zeros(49,1)];
end

predicted_dg_v3{2} = predicted;